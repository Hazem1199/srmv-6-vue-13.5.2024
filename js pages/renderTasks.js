...
async function displayTasks() {
  ...
  for (let i = 0; i < tasks.data.length; i++) {
    if (tasks.data[i].attributes.department === Dep) {
      let task = {
        id: tasks.data[i].id,
        ...
        updatedAt: tasks.data[i].attributes.updatedAt,
      };
      ...
      const formattedupdatedAt = task.updatedAt ? new Date(task.updatedAt).toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
      }) : "";
      ...
      <td class="text-center align-middle fw-bold">${task.emp} (${formattedupdatedAt})</td>
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      const taskFound = document.getElementById("taskCase").value;
